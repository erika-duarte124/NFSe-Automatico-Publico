...
# NÃO editar "True" aqui manualmente pra gerar o .exe interno — o
# build_exes.py troca esse valor sozinho na hora de compilar a edição
# "_All" e desfaz a troca logo em seguida, pra nunca ficar True no
# código-fonte versionado (o padrão público, sem limites, é sempre False).
EDICAO_INTERNA = False

LIMITE_EMPRESAS = None if EDICAO_INTERNA else 20
LIMITE_GRUPOS = None if EDICAO_INTERNA else 3
LIMITE_POR_GRUPO = None if EDICAO_INTERNA else 10
LIMITE_RODAR_AGORA = None if EDICAO_INTERNA else 5
GRUPO_PADRAO = "Grupo 1"
# dropdown de grupos: com limite, exatamente LIMITE_GRUPOS opções; sem
# limite (edição interna), uma lista generosa (não dá pra oferecer opções
# infinitas num combobox) — quem precisar de mais que isso digita direto.
GRUPOS_FIXOS = [f"Grupo {i}" for i in range(1, (LIMITE_GRUPOS or 50) + 1)]
...
def testar_webhook_chat(url: str) -> tuple[bool, str]:
    """Manda uma mensagem de teste pro webhook do Google Chat informado.
    Não acessa nada além dessa URL — só usada quando a pessoa clica em
    "Testar" na tela de Notificações."""
    if not url.strip():
        return False, "Cole a URL do webhook antes de testar."
    try:
        r = requests.post(url.strip(), json={
            "text": "✅ NFS-e Automático: notificações conectadas com sucesso a este espaço."
        }, timeout=15)
        if r.status_code < 300:
            return True, "Mensagem de teste enviada! Confira o espaço no Google Chat."
        return False, f"O Google Chat recusou a URL (HTTP {r.status_code}). Confira se copiou o link certo."
    except requests.exceptions.RequestException as e:
        return False, f"Não consegui enviar: {e}"
...
class Assistente(tk.Tk):
    def __init__(self):
        ...
        self.webhook_chat = tk.StringVar(value="")
        ...
        if not self.eh_primeira_execucao:
            ...
            self.webhook_chat.set(config_existente.get("webhook_chat", ""))
    ...
    def _montar_menu(self):
        """Barra de menu nativa (topo, alinhada à esquerda) — reúne as ações
        que não fazem parte do fluxo principal do assistente, fora da tela
        de cadastro de empresas."""
        barra = tk.Menu(self)
        self.config(menu=barra)
        menu_opcoes = tk.Menu(barra, tearoff=False, postcommand=self._atualizar_menu_opcoes)
        barra.add_cascade(label="Opções", menu=menu_opcoes)
        menu_opcoes.add_command(label="Histórico de execuções...", command=self.acao_ver_historico)
        menu_opcoes.add_command(label="Rodar agora (mês específico)...", command=self.acao_rodar_agora)
        menu_opcoes.add_command(label="Notificações...", command=self.acao_notificacoes)
        menu_opcoes.add_separator()
        menu_opcoes.add_command(label="Validação Retenções...", command=self.acao_validacao_retencoes)
        self.menu_opcoes = menu_opcoes
        self._indice_validacao_retencoes = menu_opcoes.index("end")

    def _atualizar_menu_opcoes(self):
        """Reavaliado toda vez que o menu "Opções" é aberto — a Validação
        Retenções só faz sentido depois que pelo menos 1 grupo fechou o mês."""
        estado = "normal" if self._algum_grupo_fechou_o_mes() else "disabled"
        self.menu_opcoes.entryconfig(self._indice_validacao_retencoes, state=estado)

    def _algum_grupo_fechou_o_mes(self) -> bool:
        mes_fechamento = rotina.mes_anterior(date.today())
        estado = json.loads(rotina.ARQ_ESTADO.read_text(encoding="utf-8")) if rotina.ARQ_ESTADO.exists() else {}
        grupos_estado = estado.get("grupos", {})
        return any(g.get("ultimo_fechamento_mensal") == mes_fechamento for g in grupos_estado.values())
    ...
    def mostrar_tela_empresas(self):
        ...
        texto_limite_grupo = ("sem limite de grupos/empresas" if LIMITE_GRUPOS is None
                              else f"até {LIMITE_GRUPOS} grupos, {LIMITE_POR_GRUPO} empresas cada")
        tk.Label(form, text=f"({texto_limite_grupo})",
    ...
    def _config_atual(self) -> dict:
        ...
        return {
            ...
            "webhook_chat": self.webhook_chat.get().strip(),
    ...
    def acao_adicionar(self):
        ...
        if LIMITE_EMPRESAS is not None and len(empresas_sem_esta) >= LIMITE_EMPRESAS:
            self._mostrar_limite_atingido()
            return

        grupos_existentes = {e.get("grupo", GRUPO_PADRAO) for e in empresas_sem_esta}
        if (LIMITE_GRUPOS is not None and grupo not in grupos_existentes
                and len(grupos_existentes) >= LIMITE_GRUPOS):
        ...
        if LIMITE_POR_GRUPO is not None and no_grupo >= LIMITE_POR_GRUPO:
    ...
    def acao_rodar_agora(self):
        ...
        texto_limite_rodar = ("Selecione as empresas." if LIMITE_RODAR_AGORA is None
                              else f"Selecione até {LIMITE_RODAR_AGORA} empresas.")
        ...
        tk.Label(janela, justify="center", fg="#777", text=
                 ...
                 f"de um mês que já foi fechado. {texto_limite_rodar}"
        ...
        def alternar(nome_alterado):
            ...
            if LIMITE_RODAR_AGORA is not None and len(marcadas) > LIMITE_RODAR_AGORA:
    ...
    def acao_notificacoes(self):
        """Tela de configuração do aviso no Google Chat — além da notificação
        do Windows, cada pessoa pode receber um aviso de sucesso/falha de
        cada execução automática direto num espaço do Google Chat dela."""
        janela = tk.Toplevel(self)
        janela.title("Notificações")
        janela.resizable(False, False)
        janela.grab_set()

        tk.Label(janela, text="Aviso no Google Chat", font=("Segoe UI", 12, "bold")).pack(padx=24, pady=(18, 4))
        tk.Label(janela, justify="left", wraplength=440, fg="#555", text=
                 "Além do aviso do Windows, o programa pode mandar uma mensagem pro seu "
                 "Google Chat (aparece também no Gmail) toda vez que uma retirada automática "
                 "terminar — com sucesso ou com falha.\n\n"
                 "Como pegar a URL: no Google Chat, abra (ou crie) o espaço onde quer "
                 "receber os avisos → nome do espaço → \"Apps e integrações\" → "
                 "\"Gerenciar webhooks\" → \"Adicionar webhook\" → dê um nome (ex.: "
                 "\"NFS-e Automático\") → copie a URL gerada e cole abaixo."
                 ).pack(padx=24, pady=(0, 14))

        tk.Label(janela, text="URL do webhook:").pack(anchor="w", padx=24)
        var_url = tk.StringVar(value=self.webhook_chat.get())
        tk.Entry(janela, textvariable=var_url, width=62).pack(padx=24, pady=(2, 4))

        var_status = tk.StringVar(value="")
        tk.Label(janela, textvariable=var_status, justify="left", wraplength=440).pack(padx=24, anchor="w")

        def testar():
            var_status.set("Testando...")
            janela.update()
            ok, msg = testar_webhook_chat(var_url.get())
            var_status.set(("✓ " if ok else "✗ ") + msg)

        def salvar():
            self.webhook_chat.set(var_url.get().strip())
            self._salvar_config()
            janela.destroy()

        def remover():
            var_url.set("")
            self.webhook_chat.set("")
            self._salvar_config()
            var_status.set("Webhook removido — notificações no Google Chat desativadas.")

        botoes = tk.Frame(janela)
        botoes.pack(pady=(10, 18))
        tk.Button(botoes, text="Testar", command=testar).pack(side="left", padx=(0, 8))
        tk.Button(botoes, text="Remover", command=remover).pack(side="left", padx=(0, 8))
        tk.Button(botoes, text="Salvar", font=("Segoe UI", 9, "bold"), command=salvar).pack(side="left")
        janela.transient(self)

    def acao_validacao_retencoes(self):
        """Lista, por grupo, quais empresas tiveram alguma retenção de
        imposto no mês fechado mais recente — só os nomes, a conferência
        dos valores é feita abrindo o Excel de Retenções de cada uma. Lê o
        retencoes_estado.json (gravado pelo gerar_retencoes.py a cada
        fechamento mensal), não recalcula nada na hora."""
        mes_fechamento = rotina.mes_anterior(date.today())
        estado_rotina = json.loads(rotina.ARQ_ESTADO.read_text(encoding="utf-8")) if rotina.ARQ_ESTADO.exists() else {}
        grupos_estado = estado_rotina.get("grupos", {})
        estado_retencoes = (json.loads(rotina.ARQ_RETENCOES_ESTADO.read_text(encoding="utf-8"))
                            if rotina.ARQ_RETENCOES_ESTADO.exists() else {})

        janela = tk.Toplevel(self)
        janela.title("Validação Retenções")
        janela.resizable(False, False)
        janela.grab_set()

        tk.Label(janela, text=f"Retenções destacadas — competência {mes_fechamento}",
                 font=("Segoe UI", 12, "bold")).pack(padx=24, pady=(18, 4))
        tk.Label(janela, fg="#777", font=("Segoe UI", 8), justify="center", text=
                 "Só os nomes — a conferência dos valores é feita no Excel de\n"
                 "Retenções de cada empresa (pasta do mês fechado)."
        ...
        for grupo in self._grupos_existentes():
            ...
            fechou = grupos_estado.get(grupo, {}).get("ultimo_fechamento_mensal") == mes_fechamento
            if not fechou:
                tk.Label(aba, fg="#777", justify="center", wraplength=380, text=
                         f"Esse grupo ainda não fechou o mês de {mes_fechamento}."
                         ).pack(padx=20, pady=30)
                continue

            empresas_do_grupo = [e["nome"] for e in self.empresas if e.get("grupo", GRUPO_PADRAO) == grupo]
            destacadas = [nome for nome in empresas_do_grupo
                         if estado_retencoes.get(nome, {}).get("competencia") == mes_fechamento
                         and estado_retencoes.get(nome, {}).get("total", 0) > 0]
            if not destacadas:
                tk.Label(aba, fg="#777", justify="center", wraplength=380, text=
                         "Nenhuma empresa desse grupo teve retenção nesse mês."
                         ).pack(padx=20, pady=30)
                continue

            lista = tk.Listbox(aba, width=48, height=min(10, len(destacadas)))
            lista.pack(padx=16, pady=(12, 4))
            for nome in destacadas:
                lista.insert("end", nome)
            tk.Label(aba, text="*Necessário conferência", fg="#B8860B",
                     font=("Segoe UI", 8, "bold")).pack(pady=(0, 12))

        tk.Button(janela, text="Fechar", command=janela.destroy).pack(pady=(0, 16))
        janela.transient(self)
    ...
    def _atualizar_lista(self):
        ...
        contagem = (str(len(self.empresas)) if LIMITE_EMPRESAS is None
                   else f"{len(self.empresas)}/{LIMITE_EMPRESAS}")
        self.lista_frame.config(text=f"Empresas cadastradas ({contagem})")
