...
# (a API de geração do DANFSe em /danfse/{chave} foi desativada pelo governo
# em 03/08/2026 — ver gerar_danfse.py, que monta o PDF localmente a partir
# do XML já baixado.)


def definir_ambiente(ambiente: str) -> None:
    """Resolve as URLs conforme o ambiente do config.json (padrão: producao)."""
    global URL_ADN_DISTRIBUICAO, URL_SEFIN_NFSE
...
def salvar_xml(pasta_empresa: Path, chave: str, xml: bytes, cnpj_empresa: str,
               ...
               mes_alvo: str | None = None, nsu: int | None = None) -> tuple[Path, bool]:
    ...
    if tipo_doc.upper().startswith("EVENTO"):
        # eventos podem repetir a mesma chave (mais de um evento no mesmo
        # documento); usa o NSU — estável e único na distribuição — para não
        # duplicar quando o mesmo evento é reencontrado de novo (ex.: quando
        # uma competência nova reinicia a varredura do zero e passa de novo
        # por NSUs já vistos antes por outra competência).
        nome = f"{chave}-evento-nsu{nsu}.xml" if nsu is not None else \
               f"{chave}-evento-{datetime.now():%Y%m%d%H%M%S%f}.xml"
    else:
        nome = f"{chave}.xml"

    arquivo = destino / nome
    ja_existia = arquivo.exists()
    if not ja_existia:
        arquivo.write_bytes(xml)
    return arquivo, not ja_existia


def gerar_danfse_local(xml: bytes, destino: Path) -> bool:
    """
    Gera o PDF (DANFSe) LOCALMENTE a partir do XML já baixado, seguindo o
    leiaute padronizado (Nota Técnica 008/2026). Não depende mais do ADN:
    a API oficial de geração do DANFSe foi desativada pelo governo em
    03/08/2026 (sobrecarregava a infraestrutura nacional).
    """
    try:
        pdf_bytes = gerar_pdf_danfse(xml)
    except Exception as exc:
        log(f"    DANFSe: falha ao gerar localmente: {exc}")
        return False
    if pdf_bytes is None:
        return False  # XML não é uma NFS-e (ex.: é um evento)
...
def sincronizar_empresa(empresa: dict, pasta_saida: Path, estado: dict,
    ...
    while True:
        ...
        for doc in docs:
            ...
            arquivo, novo = salvar_xml(pasta_empresa, doc["chave"], xml, cnpj, doc["tipo"],
            ...
            if novo:
                ...
                log(f"    NSU {doc['nsu']} [{doc['tipo']}]{marca} já existia -> "
                    f"{arquivo.relative_to(pasta_saida)} (pulado, sem duplicar)")

            if baixar_pdf and novo and not doc["tipo"].upper().startswith("EVENTO"):
                destino_pdf = arquivo.with_suffix(".pdf")
                if not destino_pdf.exists() and gerar_danfse_local(xml, destino_pdf):
                    total_pdf += 1
...
def completar_pdfs(empresa: dict, pasta_saida: Path,
                   competencia: str | None = None, regerar_tudo: bool = False) -> None:
    """Gera localmente o DANFSe dos XMLs já salvos. Por padrão só os que
    ainda estão sem PDF; com regerar_tudo=True, REFAZ também os que já
    existem (uso: quando o layout do gerar_danfse.py muda). Não usa mais
    rede/certificado — o PDF é montado a partir do XML que já está no disco."""
    nome = empresa["nome"]
    pasta_empresa = pasta_saida / nome
    if competencia:
    ...
    todos_xml = [xml for xml in sorted(pasta_empresa.rglob("*.xml")) if "Eventos" not in xml.parts]
    pendentes = todos_xml if regerar_tudo else [xml for xml in todos_xml if not xml.with_suffix(".pdf").exists()]
    rotulo = "PDF(s) a regerar" if regerar_tudo else "PDF(s) faltando"
    log(f"==== {nome}: {len(pendentes)} {rotulo} ====")
    if not pendentes:
        return

    gerados = 0
    for xml_path in pendentes:
        if gerar_danfse_local(xml_path.read_bytes(), xml_path.with_suffix(".pdf")):
            gerados += 1
            log(f"    PDF ok -> {xml_path.with_suffix('.pdf').relative_to(pasta_saida)}")
    log(f"  Concluído: {gerados} de {len(pendentes)} PDF(s) gerado(s).")
...
def baixar_nota_avulsa(empresa: dict, pasta_saida: Path, chave: str,
    ...
    if xml:
        arquivo, novo = salvar_xml(pasta_empresa, chave, xml, cnpj, "NFSE")
        log(f"  XML salvo em {arquivo}" if novo else f"  XML já existia em {arquivo}")
        destino_pdf = arquivo.with_suffix(".pdf")
    else:
        log("  XML não disponível por chave (a nota pode não ter sido emitida pelo "
            "Emissor Nacional). Sem XML não é possível gerar o DANFSe localmente.")
        return

    if baixar_pdf:
        if gerar_danfse_local(xml, destino_pdf):
            log(f"  PDF gerado em {destino_pdf}")
        else:
            log("  Não foi possível gerar o DANFSe a partir deste XML.")
...
def main() -> int:
    ...
    parser.add_argument("--regerar-pdf", action="store_true",
                        help="regera TODOS os PDFs a partir do XML local, mesmo os que já existem "
                             "(use quando o layout do gerar_danfse.py mudar)")
    ...
    for empresa in empresas:
        try:
            ...
            elif args.completar_pdf or args.regerar_pdf:
                completar_pdfs(empresa, pasta_saida, competencia, regerar_tudo=args.regerar_pdf)
            else:
