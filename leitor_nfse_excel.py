...
def _preencher_aba_dados(ws, registros):
    """Preenche UMA aba de dados (cabeçalho + linhas + totais) com a lista de
    registros dada — usado separadamente pra Emitidas e pra Recebidas."""
    ws.freeze_panes = "A2"
    ...
    for col_idx, (_, key, _) in enumerate(COLS, start=1):
        ...
        if key in soma_cols:
            ...
            formula = f"=SUM({col_letter}2:{col_letter}{tot_row-1})" if n_dados else 0
            cell.value = formula
            cell.number_format = FMT_MOEDA
            cell.alignment = Alignment(horizontal="right", vertical="center")
...
def gerar_excel(registros, saida_path):
    ...
    # ── Abas de Dados — separadas por papel da empresa na nota ──────────────
    emitidas = [r for r in registros if r.get("papel") == "Emitida"]
    recebidas = [r for r in registros if r.get("papel") != "Emitida"]

    ws = wb.active
    ws.title = "NFSe Emitidas"
    _preencher_aba_dados(ws, emitidas)

    ws_rec = wb.create_sheet("NFSe Recebidas")
    _preencher_aba_dados(ws_rec, recebidas)

    # ── Aba de Resumo (totais gerais — emitidas + recebidas) ─────────────────
    ...
    qtd = ws2.cell(row=3, column=1,
                  value=f"Total de notas: {len(registros)} ({len(emitidas)} emitidas, {len(recebidas)} recebidas)")
